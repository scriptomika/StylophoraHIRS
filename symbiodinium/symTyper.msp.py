...
def processClades(args, pool=Pool(processes=1)):
   ...
   logging.debug('CLADE:Processing caldes for: %s ' % args.samplesFile.name)
   # Split fasta file
   # Put all the directories is at the same level as the inFile.
   fastaFilesDir = os.path.join(os.path.dirname(args.samplesFile.name), "fasta")
   
   ######fastaList = Helpers.splitFileBySample(args.inFile.name, args.samplesFile.name, fastaFilesDir)
   fastaList = os.listdir(args.fastainputDir)
   logging.debug("Found fasta input: %s " % " ".join(fastaList))
   # Running HMMscan
   hmmerOutputDir =  os.path.join(os.path.dirname(args.samplesFile.name), "hmmer_output")
   ...
   #Parse HMMscan
   parsedHmmerOutputDir = os.path.join(os.path.dirname(args.samplesFile.name), "hmmer_parsedOutput")   
   ...
   makeCladeDistribTable( args.samplesFile.name, os.path.join(os.path.dirname(args.samplesFile.name),"hmmer_parsedOutput"))   
   generateCladeBreakdown(args.samplesFile.name, os.path.join(os.path.dirname(args.samplesFile.name),"hmmer_parsedOutput"), "HIT", 4)
   ...
   inFileRegEx = os.path.join(os.path.dirname(args.samplesFile.name),"hmmer_parsedOutput", "%s", "HIT") 
   makeDirOrdie(os.path.join(os.path.dirname(args.samplesFile.name),"hmmer_hits"))
   outputFastaRegEx = os.path.join(os.path.dirname(args.samplesFile.name),"hmmer_hits", "%s"+".fasta")
...
## MSP commented out lines with ## to resume cluster parsing without restarting CDHIT.  
...
def main(argv):
   ...
   parser_clade.add_argument('-f', '--fastainputDir', required=True,help='directory with sample fastas')
